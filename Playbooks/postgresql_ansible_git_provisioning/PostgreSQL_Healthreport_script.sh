...
mnt_pnt=`df -kh | grep -w "/" | awk  '{print$4}'`
total_m=`df -kh | grep -w "/" | awk  '{print$2}'`
latest_file=`ls -l /var/lib/pgsql/data/log | tail -1 | awk '{print$9}'`
postgres_logs=`cat /var/lib/pgsql/data/log/$latest_file`
...
html="${HOME}/health_reports/PostgreSQL-Health-Report.html"
...
echo "<fieldset>" >> $html
echo "<center>" >> $html
echo "<h2><u>PostgreSQL Deployement Report</u></h2>" >> $html
...
echo "<th>Total Memory(in MB)</th>" >> $html
echo "<th>Memory Utilization %</th>" >> $html
echo "<th>Disk Total (/) %</th>" >> $html
echo "<th>Disk Utilization (/)</th>" >> $html
echo "</tr>" >> $html
echo "</thead>" >> $html
echo "<tbody>" >> $html
echo "<tr>" >> $html
...
echo "</table>" >> $html
########################################### Disk Utilization #######################################################################
echo "<h3><u>PostgreSQL Logs</u> </h3>" >> $html
echo "<table class="pure-table">" >> $html
echo "<thead>" >> $html
echo "<tr>" >> $html
echo "<th><center>Installation Logs</center></th>" >> $html
echo "</tr>" >> $html
echo "</thead>" >> $html
echo "<tbody>" >> $html
echo "<tr>" >> $html
