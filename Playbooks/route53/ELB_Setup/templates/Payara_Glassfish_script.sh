...
total_m=`df -kh | grep -w "/" | awk  '{print$2}' | tr -d G`
os=`cat /etc/os-release | grep -iw name | awk -F "=" '{print$2}' | tr -d \"`
if [ "$os" = "SLES" ]
then
...
latest_file=`ls -l /var/lib/pgsql/data/pg_log | tail -1 | awk '{print$9}'`
postgres_logs=`cat /var/lib/pgsql/data/pg_log/$latest_file | grep -i error`
fi
cpu_ct=`cat /proc/cpuinfo | awk '/^processor/{print $3}' | wc -l`
...
html="${HOME}/health_reports/PostgreSQL-Deployment-Report.html"
...
echo "<fieldset>" >> $html
echo "<center>" >> $html
echo "<h2><u>Payara Glassfish Deployment Report</u></h2>" >> $html
...
echo "<h3><u>Payara Glassfish Instance Details</u> </h3>" >> $html
...
echo "<thead>" >> $html
echo "<tr>" >> $html
echo "<th>CPU Count</th>" >> $html
echo "<th>CPU Utilization</th>" >> $html
echo "<th>Total Memory(MB)</th>" >> $html
echo "<th>Memory Utilization (%)</th>" >> $html
echo "<th>Disk Capacity Operating System (GB)</th>" >> $html
echo "<th>Disk Utilized (GB)</th>" >> $html
echo "</tr>" >> $html
echo "</thead>" >> $html
echo "<tbody>" >> $html
echo "<tr>" >> $html
echo "<td><center>$cpu_ct</center></td>" >> $html
echo "<td><center>$cpu_pct</center></td>" >> $html
echo "<td><center>$ram_total</center></td>" >> $html
echo "<td><center>$mem_pct</center></td>" >> $html
echo "<td><center>$total_m</center></td>" >> $html
...
echo "<thead>" >> $html
echo "<tr>" >> $html
echo "<th><center>Installation Error Logs</center></th>" >> $html
echo "</tr>" >> $html
echo "</thead>" >> $html
echo "<tbody>" >> $html
echo "<tr>" >> $html
